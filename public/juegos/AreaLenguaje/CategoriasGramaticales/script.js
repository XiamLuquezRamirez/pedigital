...
function iniciarCarrera() {
  ...
  if (background === "") {
    document.body.style.animation = "moverFondo 1000s linear infinite";
  }
...
function resultadoFinal(resul) {
  ...
  if (resul == "sol") {
    ...
    document.getElementById("texto_final").innerText = "Lo siento no has logrado completar el reto. Vuelve a intentarlo";
  } else {
    document.getElementById("final").style.backgroundImage =
...
function updateNave() {
  ...
  if (navePosition + parseFloat(getComputedStyle(nave).width) >= tierraPosition + 150) {
  ...
  var solPosition = parseFloat(getComputedStyle(sol).left) + parseFloat(getComputedStyle(sol).width);
