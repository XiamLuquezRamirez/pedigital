...
<div class="row">
    ...
    <div class="col-md-3">
        <div class="form-group">
            ...
            <select class="form-control select2" data-placeholder="Seleccione" name="nom_unidad" id="nom_unidad">
                ...
                <option value="{{'Unidad '.$i}}" {{(Input::old('nom_unidad') == 'Unidad '.$i ? "selected" : "")}} selected>{{'Unidad '.$i}}</option>
                @else
                <option value="{{'Unidad '.$i}}" {{(Input::old('nom_unidad') == 'Unidad '.$i ? "selected" : "")}}>{{'Unidad '.$i}}</option>
                @endif
                @endfor
            </select>
        </div>        
    ...
    <div class="col-md-9">
        <div class="form-group">
            ...
            <select class="form-control select2" onchange="$.CargPeriodos(this.value)" data-placeholder="Seleccione" name="modulo" id="modulo">
                <option value="">Seleccione la Asignatura</option>
                @foreach ($Asigna as $Asig) {
                @if($Asig->id == $unid->modulo)
                <option value="{{$Asig->id}}" {{(Input::old('modulo') == $Asig->id ? "selected" : "")}} selected>{{$Asig->nombre.' - Grado '.$Asig->grado_modulo.'°'}}</option>
                @else
                <option value="{{$Asig->id}}" {{(Input::old('modulo') == $Asig->id ? "selected" : "")}}>{{$Asig->nombre.' - Grado '.$Asig->grado_modulo.'°'}}</option>
                @endif
                @endforeach
            </select>
    ...
    <div class="row ">
        <div class="col-md-12 col-lg-12 ">
            <div class="btn-list">
                ...
                @if($opc!='editar')
                <a class="btn btn-outline-warning" href="{{ url('/Asignaturas/NuevaUnidad') }}" title="Cancelar">
